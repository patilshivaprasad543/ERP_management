...
public class PurchaseRequestController {
    ...
    public PurchaseRequest create(@Valid @RequestBody PurchaseRequestPayload request) {
        return service.create(request.requesterEmployeeId(), request.vendorId(), request.itemDescription(),
                request.quantity(), request.estimatedUnitPrice(), request.requestedDate());
    }

    @PatchMapping("/{id}/submit")
    public PurchaseRequest submit(@PathVariable @Positive Long id) { return service.submit(id); }

    @PatchMapping("/{id}/decision")
    public PurchaseRequest decide(@PathVariable @Positive Long id, @Valid @RequestBody DecisionRequest request) {
        return service.decide(id, request.status());
    }

    @PatchMapping("/{id}/cancel")
    public PurchaseRequest cancel(@PathVariable @Positive Long id) { return service.cancel(id); }

    public record PurchaseRequestPayload(@NotNull @Positive Long requesterEmployeeId,
